Shader "Custom/Water"
{
    Properties
    {
        ...
        _Foam("Foamline Thickness", Range(0,10)) = 0.5
       	_FoamRampTex("Foam Ramp", 2D) = "white" {}
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				/*
				half4 depthSample = SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, i.scrPos);
				half depth = LinearEyeDepth(depthSample).r;

				half foamLine = 1 - saturate(_Foam * (depth - i.scrPos.w));
				half4 foamRamp = half4(tex2D(_FoamRampTex, half2(foamLine, 0.5)).rgb, 1.0); // sample the ramp texture
				
				half4 col = tex2D(_MainTex, i.uv) * foamRamp * _Tint;
				return col;
				*/
            }
            ENDCG
